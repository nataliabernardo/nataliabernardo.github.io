...
As expected, the more drastic results and more invasive, the more negative sentiment posts a procedure receives, because patients tend to expose concerns with the results. For example, although ryhoplasty presents a high "worth it" rate (90%), many patients face anxiety during the recovery period, since they can not see the final results immediately and have to wait some time until the swollen is gone.

Minimally or non-invasive treatments like coolsculpting and botox tend to receive more positive reviews. Curiously, the majority of negative reviews to coolsculpting are related to the minimal results, like in the review below:
