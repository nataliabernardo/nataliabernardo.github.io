...
I had only 2 weeks to do this task, so the model still have room for improvement
...
When a new document (review) is added to the corpus, we don't need to redo all the work. Once we build a SVM classifier, it's possible to predict the topic of this review.
