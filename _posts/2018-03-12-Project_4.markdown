...
It is challeging to extract relevant features from large number of opinions posted on review platforms and at the same time offer this information in a useful way to each user. Probabilistic topic models are becoming popular tools to uncover what other users think about a product. On the user side, it enables to personalize the content in order to shorten the time an user needs to make a decision. On the service provider's side, it can give insights of what users most care about. 
